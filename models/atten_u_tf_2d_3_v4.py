...
def precompute_freqs_cis_1d(dim: int, end: int, theta: float = 10000.0):
    positions = torch.arange(end) # len of sequence
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 2).float() / dim))
    angles = torch.outer(positions, freqs)
    cis = torch.polar(torch.ones_like(angles), angles)  # (end, dim//2)
    return cis
...
class Attn(nn.Module):
    ...
    def forward(self, x):
        ...
        self.freqs_cis = precompute_freqs_cis_1d(self.dim // self.heads, N).to(x.device) # [b, t (len), dim // heads]
        # q, k 
        q, k = map(lambda t: rearrange(t, 'b t (h d) -> b t h d', h=self.heads), qkv[:-1])
        q, k = apply_rotary_emb(q, k, freqs_cis=self.freqs_cis) # [b*nt, len, head num, dim per head]
        # reshape back
        q = rearrange(q, 'b t h d -> b h t d', b=b)
        k = rearrange(k, 'b t h d -> b h t d', b=b)
        v = rearrange(qkv[2], 'b t (h d) -> b h t d', h=self.heads) # [b, nt, len, head num, dim per head]
        ...
        # udit
        # attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1)) # [b, nt, head num, len, len]
        # logit_scale = torch.clamp(self.logit_scale, max=4.6052).exp()
        # attn = attn * logit_scale

        # to v
        attn = attn.softmax(dim=-1) # [b, nt, head num, len, len]
        x = (attn @ v) # [b, nt, head num, len, dim head]

        # out
        x = rearrange(x, 'b h t d -> b (h d) t') # [b, channel dim, real len]
        x = self.to_out(x)
...
class Mlp2D_gate(nn.Module):
    ...
    def forward(self, x):
        x_proj = self.fc1(x)                    # [B, 2*H, F, T]
        x_val, x_gate = x_proj.chunk(2, dim=1)  # split channels
        x = x_val * F.silu(x_gate)    # SwiGLU
        x = self.drop1(x)
        x = self.fc2(x)
        x = self.drop2(x)
        return x
...
class FinalLayer(nn.Module):
    ...
    def forward(self, x, c):
        ...
        return x
################################################################
class Atten_unet_2d_3_v4(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        # unet
        depth = [2,4,8,4,2] # [2,5,8,5,2] [2,2,2,2,2]
        hidden_size = config['model_params']['hidden_size'] 
        emb_dim = config['model_params']['emb_dim'] 
        num_heads = config['model_params']['num_heads'] # 4
        ...
        self.y_embedder_1 = LabelEmbedder(emb_num, emb_dim, dropout_prob=0.1) # dropout_prob=0.1
    ...
    def forward_function(self, x, t, **kwargs):
        ...
        T = x_wave.shape[-1]
        if pad_len is not None and pad_len > 0 and T > pad_len:
            x_wave = x_wave[:, :T - pad_len]
        
        return x_wave
            
        
if __name__ == '__main__':
    config_path = os.path.join('/mnt/work/diff_infer/config/atten_unet_2d_3_v4/config.toml')  
    ...
    model = Atten_unet_2d_3_v4(config["model_cfg"]).to(device)
    ...
    flops = profile_macs(model, (dummy_input, diffusion_step))
    print(f'FLOPS: {flops/1e6:.2f}, {flops/1e9:.2f}') # 8681.50
